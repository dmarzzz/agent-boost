...
test("installs a conflict-safe Hermes integration and both skills", async () => {
  ...
  assert.match(setupSkill, /use `send_message` a third[\s\S]*exact tag/u);
  ...
  assert.match(setupSkill, /three separate events/u);
  assert.match(setupSkill, /`action: send` and `target: matrix`/u);
  assert.match(setupSkill, /exact server-provided `data\.funding\.address` as the entire `message`/u);
  assert.match(setupSkill, /No label, prefix, suffix,[\s\S]*backticks, or code fence/u);
  assert.match(setupSkill, /that exact tag as the[\s\S]*entire `message`/u);
  assert.match(setupSkill, /address-only visible response/u);
  assert.match(setupSkill, /Never repeat the address/u);
  assert.doesNotMatch(setupSkill, /In the same reply, always show/u);
